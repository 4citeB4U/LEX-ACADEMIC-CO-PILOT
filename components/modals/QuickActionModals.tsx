...
const QuickNoteModal: React.FC<{ onClose: () => void }> = ({ onClose }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex justify-center items-start pt-32" onClick={handleSave}>
            <div className="bg-slate-800 rounded-xl shadow-2xl w-full max-w-xl border border-border-color p-6" onClick={e => e.stopPropagation()} onKeyDown={handleKeyDown}>
...
const QuickIntelModal: React.FC<{ onClose: () => void }> = ({ onClose }) => {
    ...
    const handleKeyDown = (e: React.KeyboardEvent) => {
     ...
     return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex justify-center items-start pt-32" onClick={onClose}>
            <div className="bg-slate-800 rounded-xl shadow-2xl w-full max-w-xl border border-border-color p-6" onClick={e => e.stopPropagation()} onKeyDown={handleKeyDown}>
